...
"""
``doctorhastie.api.report`` is a module implementing the report command.
...
def main(**kwargs):
    ...
    If Doctor Hastie fails to find a package providing the module you need, it
    ...
    if not os.path.isfile(pypifile) or not os.path.isfile(stdlibfile):
        print('You need to run "doctorhastie update" before trying to make a'
              ' report.')
        return 1

    if not os.path.isfile(os.path.join(basedir, 'setup.py')):
        print('Doctor Hastie doesn\'t support folders without a setup.py.')
        return 1
    ...
    if requirements:
        ...
        print('\n# file generated by Doctor Hastie {0}'.format(__url__))
        print('\n'.join(m.split(':')[1] for m in messages['p']))
        return 0

    print('=' * 19)
    print('Doctor Hastie Report')
    print('=' * 19)
